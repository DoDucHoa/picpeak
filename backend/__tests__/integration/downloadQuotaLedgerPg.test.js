...
maybe('download ledger on Postgres', () => {
  ...
  beforeAll(async () => {
    ...
    await admin.raw('DROP SCHEMA IF EXISTS quota_ledger_test CASCADE');
    await admin.raw('CREATE SCHEMA quota_ledger_test');
    await admin.destroy();

    pgDb = knex({
      client: 'pg', connection: PG_URL, searchPath: ['quota_ledger_test'], pool: { min: 0, max: 5 },
    });
